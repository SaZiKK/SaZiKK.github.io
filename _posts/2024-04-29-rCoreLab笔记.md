...
date modify: 2024-05-11 21:33:02 +0800
tags:
  - os
  - rCore
---
...
```
确保数组大小总是符合要求后，我们再进行数据的记录，这一步自然需要放在取/放锁和PV操作的syscall函数中实现。这里要注意两个小点，第一是need和allocation以及available的修改逻辑必须仔细思考；第二点是如果算法发现死锁风险，拒绝操作的同时，数据结构本身也需要回滚，因为我们本质上是在利用数据结构模拟系统下一刻的状态，如果下一刻的状态会产生死锁，就拒绝进入这种状态。篇幅原因下面的代码以信号量为例，锁的操作相当于容量为1的信号量，没有较大差别。
```rust
//os/src/syscall/sync.rs
